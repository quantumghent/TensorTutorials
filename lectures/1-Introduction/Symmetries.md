...
#### Tensor product and direct sum representation
...
$$
    ...
    \end{pmatrix}
$$
shows that this representation is equivalent to one where the non-trivial element of $\mathbb{Z}_2$ is represented by $H\sigma^x H^\dagger=\sigma^z$. This shows that the regular representation is equivalent to the direct sum of the trivial representation and the sign representation!
...
One of the key questions of representation theory is what the irreps of a given group are and how the tensor product of irreps (which is in general not an irrep!) decomposes in a direct sum of irreps. The latter are sometimes known as the fusion rules.


#### Example

The answer to this last question is very well understood for the case of $SU(2)$. You probably know the answer from a previous course on quantum mechanics.
...
$$
For example: $\frac{1}{2}\otimes\frac{1}{2}\simeq 0\oplus 1$.

## Symmetric tensors
TensorKit is particularly well suited for dealing with symmetric tensors. As a simple exercise, let us construct a rank 3 $SU(2)$ symmetric tensor. For example the spin $1/2$ and spin $1$ representation can be called via respectively
```{julia}
    s = SU₂Space(1/2=>1);
    l = SU₂Space(1=>1);
```
